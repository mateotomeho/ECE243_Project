//Part1 of Project ECE243: Towers of Hanoi

/*Goal make the three disks for
Small one: Blue
Medium one: Green
Large One: Red

Being able to stack the disks together depending off their size (Ex: Small over Medium, but impossible to have large over mediumn)
Being able to move them from left, middle and right using the KEY push button
Select which one the disks you are moving using the Switches for now

PS/2 keyboard control will come a bit later
*/
...
void draw(struct disk_info disks[], volatile int *KEY_ptr, volatile int *SW_ptr) {
    ...
    //Draw a limit line to simulate the rods
    draw_line(10, 220, 300, 220, 0xFFFF);
    ...
    //METHOD 2 FOR ERASING: Store the old location of boxes:

    // should check if box is at edge of the screen & change dx or dy appropriately
    //update_direction(disks); //send the address
    for (int index = 0; index < N; index++){
    ...
    //Then clear the edge-capture by writing a one into it
